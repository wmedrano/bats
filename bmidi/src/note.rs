...
impl Note {
    ...
    ///```
    /// let parsed_note = 60;
    /// let note = unsafe { bmidi::Note::from_u8_unchecked(parsed_note) };
    ///```
    ///
    /// # Safety
    ...
    /// ```
    /// use bmidi::Note;
    /// fn minor_chord(root: Note) -> Result<[Note; 3], bmidi::Error> {
...
impl TryFrom<u8> for Note {
    ...
    /// fn decode_note(number: u8) -> Result<bmidi::Note, bmidi::Error> {
    ///     let parsed_note = 60;
    ///     let note = bmidi::Note::try_from(parsed_note)?;
    ///     Ok(note)
    /// }
    ///```
    #[inline(always)]
...
impl From<Note> for u8 {
    ...
    ///```
    /// use std::convert::TryFrom;
    /// fn encode_note(note: bmidi::Note) -> u8 {
    ///     u8::from(note)
    /// }
    ///```
    #[inline(always)]
